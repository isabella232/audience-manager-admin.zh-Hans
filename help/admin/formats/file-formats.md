...
列表可用于创建[!DNL FTP]基于数据文件的宏。 某些宏可用于所有数据文件字段和行。 其他宏仅特定于标题和数据行。
...
<table id="table_A3309E175ABF4651BD11CE3632B3C553"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>订单／目标ID的别名。 </p> <p>此别名的值在目标的<span class="wintitle">外来帐户ID </span>字段中设置（在<span class="wintitle">基本设置</span>部分）。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>一个10位的UTC Unix时间戳。 </p> <p>也可以按照Java日期／时间戳格式规则将其格式化为<code>YYYYMMDDhhmmss</code>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
宏仅用于标题字段。 有关示例，请参阅[文件格式宏示例](../formats/file-format-examples.md)。
...
仅用于数据行的宏。 有关示例，请参阅[文件格式宏示例](../formats/file-format-examples.md)。
...
<table id="table_E378F94A3907407AA8110C8EE6C10909"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> <span class="term"> 数据合作伙伴唯一用户标识 </span>符。返回您分配给用户／站点访客的ID(如果该ID已与<span class="keyword">Audience Manager</span>设备ID同步)。 </p> <p>如果DPID为0，则此宏将返回<span class="keyword">Audience Manager</span> ID，而不是用户的ID。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>使用条件<code>if</code>和<code>SEGMENT_LIST</code>和<code>REMOVED_SEGMENT_LIST</code>宏时需要。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code>if(SEGMENT_LIST &amp;&amp; REMOVED_SEGMENT_LIST)endif</code> </p> </td> 
   <td colname="col2"> <p>此宏组合会创建一个条件语句，该语句列表用户属于<i>和</i>的区段已从中删除。 如果两个条件都不满足或没有数据，则返回空字符串。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>合作伙伴ID(PID)。 PID显示在管理员UI的<span class="wintitle">用户档案</span>选项卡下。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>返回列表中的区段。 接受以下可选变量： </p> 
    <ul id="ul_B111AA0D6C18445598A1444B8B7E9325"> 
     <li id="li_8603B40229624856AF1FBC434DB8F16A"> <code>segmentId</code>:旧ID。已弃用。使用<code>sid</code>（仅小写）。 </li> 
     <li id="li_1EF40DDCA3C5447586904CF021D8F912"> <code>csegid</code>:旧ID。已弃用。使用<code>sid</code>（仅小写）。 </li> 
     ...
     <li id="li_FE5049089F2944FA9DB9F9D546DBA167"> <code>alias</code>:区段的映射。已弃用。使用<code>sid</code>（仅小写）。 </li> 
    ...
    <ul id="ul_757DEB56E4F849768468F3C166B0D171"> 
     <li id="li_859E1F4F21D645519F150DC512B3EB1A"> <code>type</code>:由数字ID标识的特征类型。此变量返回： 
      <ul id="ul_C9839266783D42CCADAAC3FEA33BE4D7"> 
       ...
       <li id="li_831FF929BF50434C8804C13E5786DF79"> <code>3</code> 识别基于规则的特征(实时、;已载入 <span class="wintitle"> DCS </span>)。 </li> 
      </ul> </li> 
